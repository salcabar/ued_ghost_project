...
;(function($) {
	...
	function create_toc(opts) {
		...
		$(opts.documment_selector).find('.post-content :header').each(function() {
			console.log(this.nodeName.substring(1))
	...
	function _rename_header_content(opts ,header_obj ,level) {
		console.log(opts._headers.length,header_obj,level)
		...
		console.log(opts._headers)
		if(opts.is_auto_number == true) {
			//另存为的文件里会有编号，所以有编号的就不再重新替换

			console.log($(header_obj).text(),opts._headers.join('.'))
	...
	$.fn.ztree_toc = function(options) {
		...
		console.log($.fn.ztree_toc.defaults)
